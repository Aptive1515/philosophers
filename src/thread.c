...
/*   Updated: 2022/03/13 17:01:38 by aptive           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_test(void	*arg)
{
	...
	while (!data->philo->is_dead)
	{
		if(data->philo->is_eating)
		{
			...
			{
				//printf("Philo %d, put fork_r %p\n", data->philo->nb, data->fork_r);
				//printf("Philo %d, put fork_l %p\n", data->philo->nb, data->fork_l);
			}
		}
		...
		if (data->philo->s_time_actual.tv_usec - data->philo->s_time_last_meal.tv_usec >= 100)
			ft_died(data->philo);
	}
	return(arg);
}
